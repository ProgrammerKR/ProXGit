...
import (
	...
	proxgit_sdk "code.proxgit.io/sdk/proxgit"
)
...
// GiteaDownloaderFactory defines a proxgit downloader factory
...
func (f *GiteaDownloaderFactory) New(ctx context.Context, opts base.MigrateOptions) (base.Downloader, error) {
	...
	log.Trace("Create proxgit downloader. BaseURL: %s RepoName: %s", baseURL, repoNameSpace)
...
// NewGiteaDownloader creates a proxgit Downloader via proxgit API
//
...
func NewGiteaDownloader(ctx context.Context, baseURL, repoPath, username, password, token string) (*GiteaDownloader, error) {
	proxgitClient, err := proxgit_sdk.NewClient(
		baseURL,
		proxgit_sdk.SetToken(token),
		proxgit_sdk.SetBasicAuth(username, password),
		proxgit_sdk.SetContext(ctx),
		proxgit_sdk.SetHTTPClient(NewMigrationHTTPClient()),
	)
	if err != nil {
	...
	if err = proxgitClient.CheckServerVersionConstraint(">=1.12"); err != nil {
	...
	maxPerPage := 10
	// proxgit instances >=1.13 can tell us what maximum they have
	apiConf, _, err := proxgitClient.GetGlobalAPISettings()
	if err != nil {
		log.Info("Unable to get global API settings. Ignoring these.")
		log.Debug("proxgitClient.GetGlobalAPISettings. Error: %v", err)
	}
	if apiConf != nil {
...
func (g *GiteaDownloader) String() string {
	return fmt.Sprintf("migration from proxgit server %s %s/%s", g.baseURL, g.repoOwner, g.repoName)
}
...
// GetTopics return proxgit topics
func (g *GiteaDownloader) GetTopics(_ context.Context) ([]string, error) {
	topics, _, err := g.client.ListRepoTopics(g.repoOwner, g.repoName, proxgit_sdk.ListRepoTopicsOptions{})
	return topics, err
}
...
func (g *GiteaDownloader) GetMilestones(ctx context.Context) ([]*base.Milestone, error) {
	...
	for i := 1; ; i++ {
		// make sure proxgit can shutdown gracefully
		select {
		case <-ctx.Done():
			return nil, nil
		default:
		...
		ms, _, err := g.client.ListRepoMilestones(g.repoOwner, g.repoName, proxgit_sdk.ListMilestoneOption{
			ListOptions: proxgit_sdk.ListOptions{
				PageSize: g.maxPerPage,
				Page:     i,
			},
			State: proxgit_sdk.StateAll,
		})
		if err != nil {
			return nil, err
		}
		...
		for i := range ms {
			// old proxgit instances dont have this information
			createdAT := time.Time{}
...
func (g *GiteaDownloader) convertGiteaLabel(label *proxgit_sdk.Label) *base.Label {
	return &base.Label{
		Name:        label.Name,
		Color:       label.Color,
...
func (g *GiteaDownloader) GetLabels(ctx context.Context) ([]*base.Label, error) {
	...
	for i := 1; ; i++ {
		...
		ls, _, err := g.client.ListRepoLabels(g.repoOwner, g.repoName, proxgit_sdk.ListLabelsOptions{ListOptions: proxgit_sdk.ListOptions{
			PageSize: g.maxPerPage,
			Page:     i,
		}})
		if err != nil {
...
func (g *GiteaDownloader) convertGiteaRelease(rel *proxgit_sdk.Release) *base.Release {
	r := &base.Release{
		TagName:         rel.TagName,
...
func (g *GiteaDownloader) GetReleases(ctx context.Context) ([]*base.Release, error) {
	...
	for i := 1; ; i++ {
		...
		rl, _, err := g.client.ListReleases(g.repoOwner, g.repoName, proxgit_sdk.ListReleasesOptions{ListOptions: proxgit_sdk.ListOptions{
			PageSize: g.maxPerPage,
			Page:     i,
		}})
		if err != nil {
...
func (g *GiteaDownloader) GetIssues(_ context.Context, page, perPage int) ([]*base.Issue, bool, error) {
	...
	issues, _, err := g.client.ListRepoIssues(g.repoOwner, g.repoName, proxgit_sdk.ListIssueOption{
		ListOptions: proxgit_sdk.ListOptions{Page: page, PageSize: perPage},
		State:       proxgit_sdk.StateAll,
		Type:        proxgit_sdk.IssueTypeIssue,
	})
	if err != nil {
...
func (g *GiteaDownloader) GetComments(ctx context.Context, commentable base.Commentable) ([]*base.Comment, bool, error) {
	...
	for i := 1; ; i++ {
		...
		comments, _, err := g.client.ListIssueComments(g.repoOwner, g.repoName, commentable.GetForeignIndex(), proxgit_sdk.ListIssueCommentOptions{ListOptions: proxgit_sdk.ListOptions{
			PageSize: g.maxPerPage,
			Page:     i,
		}})
		if err != nil {
...
func (g *GiteaDownloader) GetPullRequests(_ context.Context, page, perPage int) ([]*base.PullRequest, bool, error) {
	...
	prs, _, err := g.client.ListRepoPullRequests(g.repoOwner, g.repoName, proxgit_sdk.ListPullRequestsOptions{
		ListOptions: proxgit_sdk.ListOptions{
			Page:     page,
			PageSize: perPage,
		},
...
func (g *GiteaDownloader) GetReviews(ctx context.Context, reviewable base.Reviewable) ([]*base.Review, error) {
	...
	for i := 1; ; i++ {
		...
		prl, _, err := g.client.ListPullReviews(g.repoOwner, g.repoName, reviewable.GetForeignIndex(), proxgit_sdk.ListPullReviewsOptions{ListOptions: proxgit_sdk.ListOptions{
			Page:     i,
			PageSize: g.maxPerPage,
		}})
		if err != nil {
