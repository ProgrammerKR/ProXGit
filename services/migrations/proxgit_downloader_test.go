...
func TestGiteaDownloadRepo(t *testing.T) {
	...
	proxgitToken := os.Getenv("GITEA_TOKEN")
	if proxgitToken == "" {
		t.Skip("skipped test because GITEA_TOKEN was not in the environment")
	}

	resp, err := http.Get("https://proxgit.com/proxgit")
	if err != nil || resp.StatusCode != http.StatusOK {
		t.Skipf("Can't reach https://proxgit.com, skipping %s", t.Name())
	}
	ctx := t.Context()
	downloader, err := NewGiteaDownloader(ctx, "https://proxgit.com", "proxgit/test_repo", "", "", proxgitToken)
	...
	assertRepositoryEqual(t, &base.Repository{
		...
		Owner:         "proxgit",
		IsPrivate:     false,
		Description:   "Test repository for testing migration from proxgit to proxgit",
		CloneURL:      "https://proxgit.com/proxgit/test_repo.git",
		OriginalURL:   "https://proxgit.com/proxgit/test_repo",
	...
	assert.NoError(t, err)
	sort.Strings(topics)
	assert.Equal(t, []string{"ci", "proxgit", "migration", "test"}, topics)
	...
	assertCommentsEqual(t, []*base.Comment{
		{
			...
			Content:     "a really good question!\n\nIt is the used as TESTSET for proxgit2proxgit repo migration function",
		},
		{
			IssueIndex:  4,
			PosterID:    -1,
	...
	assertPullRequestEqual(t, &base.PullRequest{
		...
		Head: base.PullRequestBranch{
			CloneURL:  "https://proxgit.com/6543-forks/test_repo.git",
		...
		PatchURL:       "https://proxgit.com/proxgit/test_repo/pulls/12.patch",
	}, prs[1])
			...
			Comments: []*base.ReviewComment{
				{
					...
					Content:   "is one `\\newline` to less?",
					TreePath:  "README.md",
					DiffHunk:  "@@ -2,3 +2,3 @@\n \n-Test repository for testing migration from proxgit 2 proxgit\n\\ No newline at end of file\n+Test repository for testing migration from proxgit 2 proxgit",
					Position:  0,
					Line:      4,
					CommitID:  "187ece0cb6631e2858a6872e5733433bb3ca3b03",
					PosterID:  689,
